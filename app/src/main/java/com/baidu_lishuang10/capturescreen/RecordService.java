...
//    private List<RecordBean> list = new ArrayList<RecordBean>();
    ...
    private void createEnvironment() {
        mVideoPath =Environment.getExternalStorageDirectory()+ File.separator;
    ...
    private void startRecord() {
        String  basePath=mVideoPath + System.currentTimeMillis();
//        videoPath=basePath+""
        videoPath=basePath + "xiang.mp4";
        audioPath=basePath+ "audio.mp4";
        mergePath=basePath +"merge.mp4";
     ...
     * 开始录音
     */
    private void startRecordRadio(File audioName) {

        mRecorder = new MediaRecorder();
        mRecorder.reset();
        mRecorder.setAudioSource(MediaRecorder.AudioSource.MIC); // 设置声音来源
        mRecorder.setOutputFormat(MediaRecorder.OutputFormat.AAC_ADTS);// 设置所录制的音视频文件的格式
        ...
        mRecorder.setOutputFile( audioName.getAbsolutePath());// 设置录制的音频文件的保存位置

        audioPath=audioName.getAbsolutePath();
//        File audioFile=new File(audioPath);
//        try {
//            audioFile.createNewFile();
//        } catch (IOException e) {
